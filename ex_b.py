...
modelo = Model("Distribuicao_Equilibrada")

# Variáveis de decisão
x = modelo.addVars(num_cidades, num_cidades, vtype=GRB.BINARY, name="x")
y = modelo.addVars(num_cidades, vtype=GRB.BINARY, name="y")
distancia_centro = modelo.addVars(num_cidades, vtype=GRB.CONTINUOUS, name="distancia_centro")
max_dist = modelo.addVar(vtype=GRB.CONTINUOUS, name="max_dist")
min_dist = modelo.addVar(vtype=GRB.CONTINUOUS, name="min_dist")

# Função objetivo: minimizar a diferença entre a distância máxima e mínima percorrida pelos centros
modelo.setObjective(max_dist - min_dist, GRB.MINIMIZE)
...
for i in range(num_cidades):
    modelo.addConstr(distancia_centro[i] == quicksum(distancias[i][j] * entregas[j] * x[i, j] for j in range(num_cidades)), f"Distancia_Centro_{i}")

# Restrição 6: a distância percorrida por cada centro deve estar entre min_dist e max_dist
for i in range(num_cidades):
    modelo.addConstr(distancia_centro[i] <= max_dist, f"Distancia_Maxima_{i}")
    modelo.addConstr(distancia_centro[i] >= min_dist, f"Distancia_Minima_{i}")
